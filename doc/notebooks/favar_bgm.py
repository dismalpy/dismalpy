...
class Iteration(tb.IsDescription):
    ...
    loadings = tb.Float64Col(shape=(647,6))
    obs_cov = tb.Float64Col(shape=(652))
...
def get_storage(file='favar_bbe.h5', replace=False, original=False):
    if replace or not os.path.isfile(file):
        ...
        h5 = tb.open_file("favar_bgm.h5", mode="w", title="FAVAR BGM Gibbs Iterations", filters=filters)
        ...
        h5 = tb.open_file("favar_bgm.h5", mode="a", title="FAVAR BGM Gibbs Iterations")
